...
export default function EditarProduto() {
  ...
  const carregarProduto = async () => {
    const { data: produto } = await supabase
      .from('produtos')
      .select('*, produto_imagens(id, url, alt_text, cor, ordem)')
      .eq('id', id)
      .single()
    ...
    if (produto) {
      ...
      if (produto.produto_imagens && produto.produto_imagens.length > 0) {
        setImagens(
          produto.produto_imagens.map((img: any) => ({
            id: img.id,
            url: img.url || '',
            cor: img.cor || '',
          }))
          ...
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-8 space-y-6">
            ...
            <div className="border-t pt-6">
              ...
              <div className="flex gap-2 mb-3">
                <input type="text" value={novaCor} onChange={(e) => setNovaCor(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), adicionarCor())} className="flex-1 px-4 py-2 rounded-xl border-2 border-gray-200 focus:border-gray-900 outline-none text-sm" placeholder="Ex: Preto" />
                <button type="button" onClick={adicionarCor} className="px-6 py-2 bg-gray-900 text-white rounded-xl text-sm font-medium hover:bg-gray-800">Adicionar</button>
              </div>
            ...
            <div className="border-t pt-6">
              <div className="flex items-center justify-between mb-3">
                <label className="block text-sm font-medium text-gray-700">📸 Imagens do Produto (uma para cada cor)</label>
                <button type="button" onClick={adicionarImagem} className="text-sm text-gray-600 hover:text-gray-900 border border-gray-300 px-3 py-1 rounded-lg">+ Adicionar Imagem</button>
              </div>
                ...
                {imagens.map((img, index) => (
                  <div key={index} className="flex gap-3 items-start p-3 bg-gray-50 rounded-xl">
                    <div className="flex-1">
                      ...
                      <input type="url" value={img.url} onChange={(e) => atualizarImagem(index, 'url', e.target.value)} className="w-full px-4 py-2 rounded-xl border-2 border-gray-200 focus:border-gray-900 outline-none text-sm" placeholder="https://i.ibb.co/sua-foto.jpg" />
                    </div>
                    <div className="w-40">
                      <label className="block text-xs text-gray-500 mb-1">Cor</label>
                      <input type="text" value={img.cor} onChange={(e) => atualizarImagem(index, 'cor', e.target.value)} className="w-full px-4 py-2 rounded-xl border-2 border-gray-200 focus:border-gray-900 outline-none text-sm" placeholder="Ex: Preto" list="cores-sugeridas" />
                      <datalist id="cores-sugeridas">
                        {coresSelecionadas.map(cor => <option key={cor} value={cor} />)}
                      </datalist>
                    </div>
                    {imagens.length > 1 && (
                      <button type="button" onClick={() => removerImagem(index)} className="mt-6 text-gray-400 hover:text-red-500 p-2">✕</button>
                    )}
                  </div>
                ))}
              </div>
            ...
            <div className="flex gap-4 pt-4">
              ...
              <Link href="/admin/dashboard" className="px-8 py-3 rounded-xl font-medium text-sm border-2 border-gray-200 text-gray-600 hover:border-gray-400 transition-colors">Cancelar</Link>
            </div>
          </form>
        </div>
      </div>
