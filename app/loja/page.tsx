...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!produto) {
    return { title: 'Produto não encontrado | JQ Confecções' }
  }

  return {
    title: `${produto.nome} | JQ Confecções`,
    ...
    <main className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div className="space-y-6">
          <div>
            <h1 className="text-3xl font-serif font-bold text-gray-900 mb-2">{produto.nome}</h1>
            <div className="flex items-baseline gap-3">
              {temPromocao ? (
                <>
                  <span className="text-3xl font-bold text-red-500">R$ {produto.preco_promocional.toFixed(2)}</span>
          ...
          <BotaoWhatsApp produto={produtoFormatado} />

          <div className="border-t pt-6">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-900 mb-3">Descrição</h3>
          ...
          {(produto.composicao || produto.instrucoes_lavagem) && (
            <div className="border-t pt-6 space-y-4">
              {produto.composicao && (
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-900 mb-2">Composição</h3>
                  <p className="text-gray-600">{produto.composicao}</p>
                </div>
              )}
              {produto.instrucoes_lavagem && (
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-wider text-gray-900 mb-2">Instruções de Lavagem</h3>
